...
**Backlog #7 — Type Slowly action.** Action for plain text that types content character-by-character via `CGEvent.keyboardSetUnicodeString` with 0.2s ± 20% jitter instead of standard paste. Useful for input fields that don't accept paste, demos, screen recordings. Architectural addition — `CommitStyle` enum in `ApplyOutcome` (standardPaste / typeSlowly / typeFast). Cancellation via Esc, progress overlay for long strings, auto-cancel on focus change. Requires AX (in Limited Mode .failed). Default ≤500 characters. ~100-150 lines + TypeSimulator.swift.
