...
enum TransformationRuntime {
    ...
    // MARK: Cyrillic ↔ Latin transliteration (via the shared ioTranslit engine)

    // The 14-language tables, detection and case-preserving mappers used to live inline here;
    // they were extracted verbatim into the `ioTranslit` package so DrPaste, PolyType (and future
    // apps) share ONE engine. These are thin adapters over its public API.

    /// Locale tie-break language id for auto-detection — forwarded to ioTranslit so DrPaste's
    /// existing behaviour (and the tests that pin it) are unchanged.
    static var localeCyrillicLanguageID: String? {
        get { Transliterator.localeLanguageID }
        set { Transliterator.localeLanguageID = newValue }
    }

    private static func cyrillicTransliterate(_ input: String) -> String {
        Transliterator.cyrillicToLatin(input)
    }

    private static func latinToCyrillicTransliterate(_ input: String, params: [String: String]) -> String {
        // params["target"] is a language id, "auto", or absent; ioTranslit maps "auto"/nil → detect.
        Transliterator.latinToCyrillic(input, target: params["target"])
    }
